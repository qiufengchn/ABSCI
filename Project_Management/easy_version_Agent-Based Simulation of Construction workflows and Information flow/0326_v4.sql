...
SELECT 
    ...
    dwh.is_valid,
    CASE
        WHEN (( SELECT min(dtr.start_day) 
                FROM daily_task_record dtr
                WHERE dtr.process = dwh.process AND dtr.floor = dwh.floor AND dtr.day_number <= dwh.day_number)) IS NULL THEN 'pending'::text
        WHEN (( SELECT min(dtr.complete_day) 
                FROM daily_task_record dtr
                WHERE dtr.process = dwh.process AND dwh.floor = dwh.floor AND dtr.complete_day IS NOT NULL AND dtr.day_number <= dwh.day_number)) IS NULL THEN 'in_progress'::text
        ELSE 'completed'::text
    END AS task_status,
    dwh.recorded_at,
    LEAST(dwh.planned_remaining, p.initial_production_rate) AS actual_work_done,
    (p.initial_production_rate - dwh.planned_remaining) AS efficiency_perturbation,
    (LEAST(dwh.planned_remaining, p.initial_production_rate) / p.initial_production_rate) * 100 AS actual_efficiency_percentage
FROM 
    daily_work_history dwh
JOIN 
    process p ON dwh.process = p.process
ORDER BY 
    dwh.day_number, dwh.process, dwh.floor;

SELECT * FROM public.daily_task_details;

SELECT * FROM project_progress_details;