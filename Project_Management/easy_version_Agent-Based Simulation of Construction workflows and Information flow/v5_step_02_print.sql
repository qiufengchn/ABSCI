...
WITH calculated_efficiency AS (
    SELECT
        ...
        END AS remain_work,
        -- 新增的列：当前完成的工作
        dwh.planned_remaining - COALESCE((SELECT SUM(daily_work_done) 
                                           FROM daily_task_record dtr 
                                           WHERE dtr.process = dwh.process 
                                             AND dtr.floor = dwh.floor 
                                             AND dtr.day_number <= dwh.day_number), 0) AS currently_completed_work,
        -- 新增的列：未改变的工作量
...
-- 列currently_completed_work，其值为本行的planned_remaining减去remain_work的数值。

-- 列unchanged_quantity_of_work，根据本行的process名称从task表格中读取对应的og_initial_quantity数据。

-- 在CREATE View的时候，增加一个新的列名称为today_workload，数值为同一floor的同一个process前一天的remain_work减去当天的remain_work