...
SELECT
    ...
    CASE 
        ...
        THEN calculated_efficiency.actual_efficiency + 
             (calculated_efficiency.planned_remaining - COALESCE((SELECT SUM(daily_work_done) 
                                                  FROM daily_task_record dtr 
                                                  WHERE dtr.process = calculated_efficiency.process 
                                                    AND dtr.floor = calculated_efficiency.floor 
                                                    AND dtr.day_number <= calculated_efficiency.day_number), 0))
...
-- 有问题，Partition phase 2 的结果不对，需要重新检查。updated_actual_efficiency 的定义